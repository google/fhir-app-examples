...
package com.google.fhir.examples.cloudfunction.data

import com.google.android.fhir.sync.DownloadWorkManager
import com.google.android.fhir.sync.Request
import com.google.android.fhir.sync.SyncDataParams
import com.google.fhir.examples.cloudfunction.DemoDataStore
...
class TimestampBasedDownloadWorkManagerImpl(private val dataStore: DemoDataStore) :
  ...
  private val urls =
    LinkedList(
      listOf(
        "Patient?address-city=BOSTON&_sort=_lastUpdated",
  ...
  override suspend fun getSummaryRequestUrls(): Map<ResourceType, String> {
    return urls.associate {
      ResourceType.fromCode(it.substringBefore("?")) to
        it.plus("&${SyncDataParams.SUMMARY_KEY}=data")
    }
  }
  ...
  override suspend fun processResponse(response: Resource): Collection<Resource> {
    ...
    if (response is Bundle && response.type == Bundle.BundleType.SEARCHSET) {
      bundleCollection =
        response.entry.map { it.resource }.also {
          extractAndSaveLastUpdateTimestampToFetchFutureUpdates(it)
        }
    }
...
private fun affixLastUpdatedTimestamp(url: String, lastUpdated: String): String {
  ...
  if (downloadUrl.contains("&_page_token")) {
    downloadUrl = url
  }
